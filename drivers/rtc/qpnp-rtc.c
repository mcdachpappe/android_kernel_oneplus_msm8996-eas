			...
			u16 base, int count)
{
    ...
    if (base == (rtc_dd->alarm_base + REG_OFFSET_ALARM_CTRL1)) {
	    dev_dbg(rtc_dd->rtc_dev, "write ALARM_CTRL1=0x%x\n", *rtc_val);
    }
...
qpnp_rtc_alarm_irq_enable(struct device *dev, unsigned int enabled)
{
	...
	ctrl_reg = enabled ? (ctrl_reg | BIT_RTC_ALARM_ENABLE) :
				(ctrl_reg & ~BIT_RTC_ALARM_ENABLE);
    dev_dbg(rtc_dd->rtc_dev, "%s,%d write ALARM_CTRL1=0x%x\n",__func__,__LINE__,ctrl_reg);
	rc = qpnp_write_wrapper(rtc_dd, &ctrl_reg,
			rtc_dd->alarm_base + REG_OFFSET_ALARM_CTRL1, 1);
	if (rc) {
...
static int qpnp_rtc_probe(struct spmi_device *spmi)
{
	...
	if (rc) {
	    dev_dbg(&spmi->dev, "Unable to read RTC time\n");
		return rc;
	}

	rc = qpnp_rtc_read_alarm(rtc_dd->rtc_dev, &alarm);
	if (rc) {
		dev_dbg(&spmi->dev, "Unable to read RTC alarm time\n");
		return rc;
	}
	...
	if (alarm.enabled && rtc_tm_to_ktime(now).tv64 >
		...
		if (rc) {
			dev_dbg(&spmi->dev, "Unable to disable alarm irq\n");
			return rc;
		}
	}
...
static void qpnp_rtc_shutdown(struct spmi_device *spmi)
{
    ...
    dev_dbg(&spmi->dev, "Alarm set for - h:r:s=%d:%d:%d, d/m/y=%d/%d/%d\n",
	        alarm.time.tm_hour, alarm.time.tm_min,
	        alarm.time.tm_sec, alarm.time.tm_mday,
	        alarm.time.tm_mon, alarm.time.tm_year);
